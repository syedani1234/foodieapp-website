...
export const fetchRestaurantsByCuisineSlug = async (cuisineSlug) => {
  try {
    ...
    const normalizedData = {
      ...
      count: data.count || (Array.isArray(data.restaurants) ? data.restaurants.length : 0)
    };
    
    console.log(`âœ… Found ${normalizedData.restaurants.length} restaurants for ${cuisineSlug}`);
    return normalizedData;
    
  } catch (error) {
    console.error(`âŒ Error fetching restaurants for cuisine "${cuisineSlug}":`, error);
    
    // Provide specific error messages based on status
    if (error.response?.status === 404) {
      throw new Error(`Cuisine "${cuisineSlug}" not found. It may have been removed or the URL is incorrect.`);
    }
    
    if (error.response?.status === 500) {
      throw new Error(`Server error while fetching ${cuisineSlug} restaurants.`);
    }
...
export const fetchRestaurantDetails = async (restaurantId) => {
  try {
    ...
    console.error(`âŒ Error fetching restaurant details for ID ${restaurantId}:`, error);
...
export const getCuisineBySlug = (cuisines, slug) => {
  ...
  return cuisines.find(c => 
    c && (c.slug === slug || nameToSlug(c.name) === slug)
  );
};
...
export const sortCuisines = (cuisines, sortBy = 'name') => {
  ...
  switch (sortBy) {
    ...
    case 'restaurants':
      return sorted.sort((a, b) => (b.restaurant_count || 0) - (a.restaurant_count || 0));
    case 'featured':
