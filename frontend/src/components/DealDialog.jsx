...
const DealDialog = ({ open, onClose, deal, onCustomizationComplete, existingCustomization }) => {
  ...
  const generateDealItems = () => {
    ...
    const originalPrice = deal.original_price || deal.discount_price || deal.price || 0;
    ...
    switch (dealType) {
      ...
      case 'burger':
        ...
        for (let i = 1; i <= burgerCount; i++) {
          items.push({
            id: `burger-${i}`,
            type: 'burger',
            name: burgerCount > 1 ? `Classic Beef Burger (${i} of ${burgerCount})` : 'Classic Beef Burger',
            description: burgerCount > 1 ? `${burgerCount} Classic Beef Burgers` : 'Juicy beef burger',
      ...
      case 'pizza-burger-combo':
        ...
        items.push({
          ...
          base_price: Math.floor((pizzaBasePrice / comboTotalOriginal) * dealPrice),
        ...
        items.push({
          ...
          base_price: Math.floor((burgerBasePrice / comboTotalOriginal) * dealPrice),
  ...
  const parseBurgerCount = (title, description) => {
    ...
    if (text.includes('2 burgers') || text.includes('2 burger') || 
        text.includes('two burgers') || text.includes('two burger') ||
        (text.includes('2') && text.includes('burger'))) {
      return 2;
    }
    
    // Check for "3 burgers" or similar patterns
    if (text.includes('3 burgers') || text.includes('3 burger') || 
        text.includes('three burgers') || text.includes('three burger')) {
      return 3;
    }
    ...
    if (open && deal) {
      ...
      dealItems.forEach(item => {
        initialCustomization[item.id] = {
          ...
          selectedToppings: [],
          selectedAddons: [],
          ...(existingCustomization?.items?.find(i => i.itemId === item.id)?.customization || {})
        };
      });
  ...
  const calculateItemTotal = (item) => {
    ...
    // Add size price
    const selectedSize = item.options?.sizes?.find(s => s.id === itemCustomization.selectedSize);
    if (selectedSize) {
    ...
    if (item.type === 'pizza') {
      const selectedCrust = item.options?.crusts?.find(c => c.id === itemCustomization.selectedCrust);
      if (selectedCrust) {
  ...
  const handleAddToCart = () => {
    ...
    setSuccessMessage(existingCustomization ? 'Deal updated!' : 'Added to cart!');
                ...
                {item.options.sizes.map((size) => (
                  <Grid item xs={12} sm={4} key={size.id}>
                    <Card 
                      ...
                      sx={{
                        cursor: 'pointer',
                        border: itemCustomization.selectedSize === size.id ? 
                          '2px solid brown' : '1px solid #ddd',
                        bgcolor: itemCustomization.selectedSize === size.id ? 
                          '#f5e6d3' : 'transparent'
                      }}
                ...
                {item.options.addons.map((addon) => (
                  <Grid item xs={12} sm={6} key={addon.id}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={(itemCustomization.selectedAddons || []).includes(addon.id)}
        ...
        <Box mt={3} p={2} bgcolor="grey.50" borderRadius={1}>
          ...
          {calculateItemTotal(item) !== item.base_price && (
            <Box display="flex" justifyContent="space-between" mb={0.5}>
              ...
              <Typography variant="body2" color={calculateItemTotal(item) > item.base_price ? "success.main" : "error.main"}>
                {calculateItemTotal(item) > item.base_price ? '+' : ''}
...
<Card 
  ...
  sx={{
    cursor: 'pointer',
    border: itemCustomization.selectedSize === size.id ? 
      '2px solid orange' : '1px solid #ddd',
    bgcolor: itemCustomization.selectedSize === size.id ? 
      '#fff3e0' : 'transparent'
  }}
  onClick={() => handleSizeChange(item.id, size.id)}
>
  <CardContent sx={{ textAlign: 'center', py: 2 }}> {/* Fixed line */}
    <Typography variant="subtitle2" fontWeight="bold">
      {size.name}
    </Typography>
    {size.price > 0 && (
                ...
                {item.options.crusts.map((crust) => (
                  <Grid item xs={12} sm={4} key={crust.id}>
                    <Card 
                      ...
                      sx={{
                        cursor: 'pointer',
                        border: itemCustomization.selectedCrust === crust.id ? 
                          '2px solid orange' : '1px solid #ddd',
                        bgcolor: itemCustomization.selectedCrust === crust.id ? 
                          '#fff3e0' : 'transparent'
                      }}
                ...
                {item.options.toppings.map((topping) => (
                  <Grid item xs={12} sm={6} md={4} key={topping.id}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={(itemCustomization.selectedToppings || []).includes(topping.id)}
                          onChange={() => handleToppingToggle(item.id, topping.id)}
                          color="primary"
                        />
                      }
                      label={
  ...
  const renderStepContent = () => {
    ...
    switch (activeStep) {
      case 0: // Select Quantity
        return (
          <Box>
            <Paper sx={{ p: 3, mb: 3, bgcolor: 'primary.50', border: '1px solid', borderColor: 'primary.100' }}>
              ...
              {burgerCount > 1 && (
                <Alert severity="info" sx={{ mt: 2 }}>
                  <Typography variant="body2">
                    This deal contains <strong>{burgerCount} burgers</strong>. Each burger can be customized separately.
                  </Typography>
                </Alert>
              )}
            </Paper>
              ...
              <List>
                {dealItems.map((item, index) => (
                  <ListItem 
                    key={index}
                    sx={{ 
                      borderBottom: index < dealItems.length - 1 ? '1px solid #eee' : 'none',
                      py: 2
                    }}
                  >
                    <ListItemText 
                      primary={
                        <Box display="flex" alignItems="center">
                          {item.type === 'pizza' && <LocalPizza sx={{ mr: 2, color: 'orange.main' }} />}
                          {item.type === 'burger' && <Fastfood sx={{ mr: 2, color: 'brown.main' }} />}
            ...
            <Paper sx={{ p: 3 }}>
              ...
              <Box display="flex" alignItems="center" justifyContent="center" mb={3}>
                ...
                <Typography variant="h3" sx={{ minWidth: '80px', textAlign: 'center' }}>
                  {dealQuantity}
                </Typography>
              ...
              <Box>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  <Typography variant="body1">Rs. {dealPrice.toFixed(2)}</Typography>
                </Box>
          ...
          <Box>
            <Alert severity="info" sx={{ mb: 3 }}>
              <Typography variant="body2">
                ...
                {burgerCount > 1 && (
                  <><br /><strong>Note:</strong> This deal contains {burgerCount} burgers. Customize each burger separately.</>
                )}
              </Typography>
            </Alert>
            ...
            {dealItems.map((item, index) => (
              <Paper 
                ...
                sx={{ 
                  ...
                  mb: 3, 
                  border: '2px solid',
                  borderColor: item.type === 'pizza' ? 'orange.main' : 
                             item.type === 'burger' ? 'brown.main' : 'primary.main'
                }}
              >
                {item.type === 'burger' ? (
            ...
            <Paper sx={{ p: 3, mt: 2 }}>
              ...
              <Box display="flex" justifyContent="space-between" mb={1}>
                ...
                <Typography variant="body2">Rs. {(dealPrice * dealQuantity).toFixed(2)}</Typography>
              </Box>
              ...
              <Box display="flex" justifyContent="space-between" mb={1}>
                ...
                <Typography variant="body2">Rs. {finalTotal.toFixed(2)}</Typography>
              </Box>
          ...
          <Box>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              {dealItems.map((item, index) => {
                ...
                return (
                  <Box key={item.id} mb={3} pb={2} borderBottom={index < dealItems.length - 1 ? '1px dashed #ddd' : 'none'}>
                      ...
                      <Box flex={1}>
                        ...
                        <Box ml={2} mt={1}>
                          ...
                          {itemCustomization.selectedSize && (
                            <Typography variant="body2" color="text.secondary">
                              â€¢ Size: {itemCustomization.selectedSize.toUpperCase()}
                            </Typography>
                          )}
                          
                          {/* Crust for pizza */}
                          {item.type === 'pizza' && itemCustomization.selectedCrust && (
                            <Typography variant="body2" color="text.secondary">
                          ...
                          {/* Toppings for pizza */}
                          {item.type === 'pizza' && itemCustomization.selectedToppings && itemCustomization.selectedToppings.length > 0 && (
                            <Typography variant="body2" color="text.secondary">
                              â€¢ Toppings: {itemCustomization.selectedToppings.map(toppingId => {
                                const topping = item.options?.toppings?.find(t => t.id === toppingId);
                                return topping?.name;
                              }).filter(Boolean).join(', ')}
                            </Typography>
                          ...
                          {/* Addons for burger */}
                          {item.type === 'burger' && itemCustomization.selectedAddons && itemCustomization.selectedAddons.length > 0 && (
                            <Typography variant="body2" color="text.secondary">
                              â€¢ Extras: {itemCustomization.selectedAddons.map(addonId => {
                                const addon = item.options?.addons?.find(a => a.id === addonId);
                                return addon?.name;
                              }).filter(Boolean).join(', ')}
                            </Typography>
              ...
              <Box>
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  <Typography variant="body1">Rs. {(dealPrice * dealQuantity).toFixed(2)}</Typography>
                </Box>
                ...
                <Box display="flex" justifyContent="space-between" mb={1}>
                  ...
                  <Typography variant="body1">Rs. {finalTotal.toFixed(2)}</Typography>
                </Box>
            ...
            <Alert severity="success">
              <Box display="flex" alignItems="center">
                <Restaurant sx={{ mr: 1 }} />
                <Typography variant="body2">
                  Prepared by <strong>{deal.restaurant_name || deal.restaurant || "Restaurant"}</strong>
                </Typography>
              </Box>
            </Alert>
          </Box>
